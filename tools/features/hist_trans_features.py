...
def e001_basic_numerical_features(df):
    # define aggregations
    e001_aggs = {
        'card_id': ['size'],
    ...
    features = df.groupby('card_id').agg(e001_aggs)
    ...
    features = features.add_prefix(
        'e001_hist_trans_basic_numerical_').reset_index(drop=False)
    return features


def e002_basic_categorical_features(df):
    ...
    features = df.groupby('card_id').agg(e002_aggs)
    features.columns = [col[0] + '_' + col[1] for col in features.columns]
    # feature engineering after aggregations
    features = features.add_prefix(
        'e002_hist_trans_basic_categorical_').reset_index(drop=False)
    return features


def _hist_trans_features(df, exp_ids):
    _features = []
    if 'e001' in exp_ids:
        _features.append(e001_basic_numerical_features(df))
    if 'e002' in exp_ids:
        _features.append(e002_basic_categorical_features(df))
    features = pd.concat(_features, axis=1)
    return features


@dec_timer
def _load_hist_trans_features_src(
    ...
    series_path = './inputs/prep/historical_transactions_v1.pkl.gz'
